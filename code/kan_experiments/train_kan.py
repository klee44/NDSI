...
#print(data[i])
'''
print(len(data[26]['solutions'][0][0]['t']))
print(len(data[26]['solutions'][0][0]['y'][0]))
print(len(data[26]['solutions'][0][0]['y'][1]))
print(len(data[26]['solutions'][0][1]['t']))
print(len(data[26]['solutions'][0][1]['y'][0]))
print(len(data[26]['solutions'][0][1]['y'][1]))
'''
...
for i in range(63):
	...
	for j in range(2):
		...
		print(exp_path)
		ckpt_path = os.path.join(exp_path, "model_kanlayer_3_" + str(seed) + '.ckpt')
		fig_save_path = os.path.join(exp_path,"model_kanlayer_3_"+str(seed))
...
'''
print(ckpt_path)
t = np.array(data[26]['solutions'][0][0]['t'])
print(t.shape)

x = np.expand_dims(np.array(data[26]['solutions'][0][0]['y'][0]),-1)
y = np.expand_dims(np.array(data[26]['solutions'][0][0]['y'][1]),-1)
xy = np.concatenate((x,y), 1)

t = torch.tensor(t).squeeze()

train_data = torch.tensor(xy).unsqueeze(0)
...
for i in range(2):
	axes.append(fig.add_subplot(1,2,i+1))
	axes[i].plot(t,train_data[0,:,i],lw=3,color='k')
	axes[i].plot(t,pred_y.detach().numpy()[0,:,i],lw=2,color='c',ls='--')
